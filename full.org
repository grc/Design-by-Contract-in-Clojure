...
#+BEGIN_SRC clojure :tangle yes :exports code
  (defn wrap-arg-contract [arg-contract]
    ...
    (let [arg (first arg-contract)
      ...
      (wrap contract arg (:post-message contract) (:pre-message contract) )))
  ...
  (def faulty-sqrt-2
    (wrap-arg-contract (list (fn [not-used] -1) (make-contract gt0? gt0?))))
  
  (deftest faulty-sqrt-2-test
    (is (thrown-with-msg? java.lang.Exception #"Post condition" (faulty-sqrt-2 1)))
    (is (thrown-with-msg? java.lang.Exception #"Pre condition" (faulty-sqrt-2 0))))
  
  
#+END_SRC


The call to =resolve= in =wrap-arg-contract= deserves some
explanation.  As described earlier \ref{clojure-section}, symbols are
resolved at compile time to locate the relevant =var= --- the
function which will be invoked.  In our =wrap-arg-contract= function
we only receive the symbol representing the desired function at run
time.  =resolve= provides a run time means of accessing the required
=var=, allowing us to invoke the  contract predicate.
