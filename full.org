...
or an expression of the form =(x .\\nbsp y)= where =x= and =y= are
themselves sexprs.  The notation =(x .\\nbsp y)= denotes a cons cell,
terminology derived from the lisp function used to construct it.  It
represents an ordered pair of values and is typically used to build
linked lists.  In such a case the second value (known as the =cdr= of
the cell) points to the next cons cell.  Proper lists are terminated
when the =cdr= is nil. Figure \ref{fig:cons} shows the construction of
a proper list =(x .\nbsp (y .\nbsp nil))=
...
=(x . (y . nil))= is usually represented as =(x y)=.
