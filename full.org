...
\hypersetup{ colorlinks,% citecolor=black,% filecolor=black,%
    linkcolor=black,% urlcolor=black, pdfauthor=Giles Chamberlin,
...
developed to provide Clojure with support for Design by Contract using run-time 
assertion checking. This framework 
...
complicated as all the easy problems have been solved.
...
Our aim in this dissertation is to develop a framework which will
provide support for Design by Contract in Clojure.  Clojure is a
relatively recent addition to the set of functional languages.  A
member of the Lisp family of languages, it uses the Java Virtual
Machine and so can take advantage of the plethora of Java libraries.

In this dissertation we first review the background to Design by
Contract, its inception, current implementations and extension into
the world of functional programming.  Having outlined design goals for
our framework we will look in some detail at two possible
...
debug print statements is wide spread, particularly in the case of a
...
such as syntax highlighting and auto-completion, are no longer
...
#+latex: \begin{listing}[H]
#+LATEX: \caption{Findler and Felleisen's \texttt{wrap} meta-function} \label{ff-wrap}
#+begin_example
wrap: t contract -> t -> string -> string -> t
wrap = fix wrap \ ct \ x \ p \ n
  if flatp(ct) then 
...
Findler and Felleisen \cite{hof} also investigates blame assignment
and introduces the concept of positive and negative positions in the
\texttt{wrap} function.  Initially these are applied to caller and
called function. Should a first order contract fail the positive
location is blamed.  Higher-order contracts have the positive and
negative locations swapped before the recursive application of
\texttt{wrap}.
...
not appear in the list of blame candidates.  More importantly we are
limited to providing first-order checks on the values, a severe
limitation in a functional world where higher-order parameters are a
frequent occurrence.
...
(declare make-contract dom rng flat? lenient strict pred contract-error)
...
=make-contract=: =make-contract/2= is a simple shorthand version
which calls =make-contract/4= with default values of the message to
...
checks would limit us to simple flat assertions about the return value,
...
#+latex: \begin{listing}[H]
#+latex: \caption{Supporting functions for \texttt{attach-contracts-1}}
#+BEGIN_SRC clojure :tangle yes :exports code
...
#+latex: \begin{listing}[H]
#+latex: \caption{Macro expansion of \texttt{attach-contracts-1}}\label{macroexpand} 
#+BEGIN_SRC clojure :tangle no :exports code 
...
(def two-arg-contract (make-contract [lenient lenient] gt0?))
...
=attach-contracts-2= is evaluated, the binding has been exited and
=*contracted-function-args*= no longer holds the relevant argument
vector.  We have essentially encountered the same problem as in the C
libraries where some functions set =errno= with important information.
If you don't check in time it's lost.  As our design ethos revolves
around delayed evaluation, checking in time is not possible.
...
A brief explanation of Clojure's syntax is useful here. =(.method
object args)= is syntactical sugar for a Java call
=object.method(args)= so the =callers= function above creates a new
=Throwable= object and populates its stack trace: a typical Java
solution to the problem.  The final line of =callers= prunes the stack
trace, removing function calls which are an artefact of the
...
=:contracts= meta data of the given function.

#+begin_src clojure :tangle yes :exports code
(defn contracts
  "Prints any contract documentation associated with `f'."
  [f]
  (print (:contracts (meta f))))


#+end_src

We now have to store some useful information in the =:contracts= meta
...
names  of the contracts themselves.  To do this we use a call
to =vary-meta= to associate the stringified version of the contracts
with the contracts meta-data of or function.  The 

#+begin_src clojure :tangle yes :exports code
(defn contract-name [f]
  (let [name (:name meta f)]
    (if name
      name
      "name unknown")))

(defn dom-rng-contracts
  "Extract domain and range contracts from `c', assumed to be built
   with `make-contract'."
  [c]
  (let  [dom-contracts (map contract-name (dom c))
         dom-as-str (map (fn [s] (format "%s " s)) dom-contracts) 
         rng-contract (contract-name (rng c))]
    (format
     "Domain contracts: %s\nRange contract %s\n"
     (apply str dom-as-str)
     rng-contract)))

(defn decompose-contract
  "Deconstructs the contract block passed in to it, returning concatenated
   string versions of the components."
  [c]
  (cond
    (vector? c) (apply str  (map decompose-contract c))
    (map? c) (format "Higher-order parameter:\n %s\n" (dom-rng-contracts c))
    (function? c) (format "First-order parameter: %s\n\n" (contract-name c))))
...
(defmacro attach-contracts [newname func contracts post-condition]
  ...
  `func' with the args stored in arg/contracts wrapped in
   the corresponding contract."
  (let [args (vec (map gensym  (range (count contracts))))
        contract-docs (decompose-contract contracts)]
    `(def ~newname
       (vary-meta
        (fn ~args 
          (let [wrapped-args# 
...
endpoints (analogous to a telephone) to place calls into the system.
Interoperability between our equipment and that of the various endpoint
...
device specific plug-in functions can be written.  These may be passed
...
As the plug-in functions will be written over a period of time by a
...
valuable, though only if the plug-in writer pays attention to said
documentation.

By providing a contract on the functions taking plug-ins as a parameter
we provide not only documentation as to expected plug-in arguments and
...
plug-in functions.  The plug-in should take two parameters, one of
...
possible implementations come to mind: we could either take a map of
endpoints and setup functions as an argument, or refer to a global
value. In keeping with Clojure's functional nature we choose the
former. Our abstract functions, of which =call-setup-request= is the
first, will therefore take a map of ={type, function}= and an endpoint
type as their first arguments.  The author of a plug-in will be
required to register his function in that map, using our registration
function.
...
(defn call-setup-request [function-map endpoint-type endpoint url]
  (let [plug-in-fn (endpoint-type function-map)]
    (plug-in-fn endpoint url)))
...
indicated, exonerating the plug-in developer.  In the case of
=faulty-setup-request=, a post-condition is indicated, blaming the
developer of the plug-in for the error.
...
languages, this still feels somewhat fragile.  One potentially
fruitful area of improvement is the structure of the contracts
themselves.  The nested maps and vectors prove somewhat cumbersome to
handle and would benefit from either a library of helper functions to
manipulate them, or an alternative, more convenient, data type representation.
...
We have surveyed the existing state of Design by Contract in both
object-oriented and functional languages.  With this background we
have developed a Clojure framework which supports Design by Contract
on higher order functions.  Having implemented a worked example based
on a real world scenario, we have reflected on the strengths and
weaknesses of the work and identified a number of areas of
investigation.
