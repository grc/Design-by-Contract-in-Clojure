...
Writing a correctly functioning computer program is hard.  A number
of tools and techniques have evolved which help the programmer
maintain a degree of control over the burgeoning complexity of the
system he is developing.  In response, the systems we are trying to
build have become still more complicated.

One are of increasing complexity is the growth of parallelism in many
...
count.  Intel's  next generation Boxboro  chip provides 20 concurrent
threads of execution.
\cite{http://www.intel.co.uk/content/www/us/en/processors/public-roadmap-article.html}.
For a single program to take advantage of this power it must be able
to operate in parallel.  But writing correct parallel programs is
hard.

Functional programming provides one approach to reducing the
difficulty parallel programming.  As a pure functional program has no side
...
terminated.  Assertions were taken further by Meyer's ideas of Deign
...
This dissertation seeks to take some of the work on Design by
Contract in a functional world and apply that to Clojure, a
relatively new functional language in the Lisp family.
