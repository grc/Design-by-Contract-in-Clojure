...
(defn ho-wrap [ct x p n]
  (let [d (dom ct)
    ...
    (cond
      ...
      (= 2 arity)
      (fn [a b] (wrap-x r
                        (x (wrap-x (vector (get d 0)) a n p) (wrap-x (vector (get  d 1 )) b n p))
                        p
                        n)))))
...
Considering now higher order functions, we will introduce
=invoke-two-arg=, a function which takes as its single parameter a
function of arity two.  It applies this function to fixed arguments.
We also introduce =two-arg-contract= which will constrain the
function parameter definition: it will take any arguments but must
...
(attach-contracts two-arg-c two-arg [two-arg-contract])

(deftest two-arg-test
  (is (= 3 (two-arg-c +)))
  (is (thrown-with-msg? java.lang.Exception #"Post") (two-args-c -) ))
...
We will then provide a series of device specific functions to
request an endpoint to make a call setup attempt. These will use
simple first order contracts to check their pre and post conditions:
have they been given a valid endpoint address and a valid SIP URI, are
they returning an agreed value.
...
communications.  Call set up request are not the same as successful
...
We now provide the abstract call setup request function.  All of the
abstract functions will take the endpoint type as their first
argument.  This will be used to discover the appropriate function in a
global map of setup request implementations indexed by type.  Th
author of a plugin will be required to register his function in that map. 
...
TODO ensure this is in outline on clojure

A brief recap on Clojure's approach to concurrency and mutable data:
most objects within Clojure are immutable, mutabilty must be
explicit.  The call to =(ref {})= in =def-concrete-setup-request=
establishes a named mutable reference to an, initially empty, map.
=add-setup-request-impl= provides a means of updating that map,
=dosync= provides a transaction within which =alter= invokes =conj=
on =concrete-setup-request=, appending the map ={type fn-impl}=.  Within
=call-setup-request= we access the current value of
concrete-setup-request via the invocation of =deref=.
...
parameter should take two arguments: a IPv4 address and a SIP URI.
We have already considered predicates for the ranges of those two
arguments but are in danger of missing the assumption on the arity
=fn=.  Functions declared through use of the =defn= macro contain a
...
We therefore define a contractual version of =add-setup-request-impl=
conventionally named =add-setup-request-impl-c=.  We have restricted
access to the non contractual version by declaring it private to our
name space through the use of =defn-= rather than =defn=.  It should
be noted that this provides protection against inadvertent rather than
malicious invocation.

=add-setup-request-impl= takes two arguments.  We wish to place a
contract of =callable?= on the first; the parameter must form part of
a valid sexpr when appearing as the first element of a list.  The
second argument must be a function, of assumed arity 2, taking both
an IPv4 address and a SIP URI, returning a =code-setup-request-result=.
...
(def setup-request-contract (make-contract-1 [ip-address? sip-uri?] call-setup-request-result?))
...
(defn faulty-return-value [ip-address uri]
  ;; Product specific code omitted
  0)

(deftest setup-request-test
  (add-setup-request-impl-c :hdx 'hdx-setup)
  (is (= :ok (call-setup-request :hdx "192.168.10.1" "me@example.com" ))))
...
Ths one works!
(clojure.core/defn
 add-setup-request-impl-c
 [callable?4395 c?4396]
 (clojure.core/let
  [wrapped-args__4305__auto__
   (clojure.core/map
    dbc.core/wrap-arg-contract
    (dbc.core/zip [callable?4395 c?4396] [[callable?] c?]))]
  (clojure.core/assert add-setup-request-impl)
  (clojure.core/assert wrapped-args__4305__auto__)
  (clojure.core/print
   (clojure.core/format
    "attach-contracts calling original with %s"
    wrapped-args__4305__auto__))
  (clojure.core/apply
   add-setup-request-impl
   wrapped-args__4305__auto__)))
...
TODO: above fails because 

(clojure.pprint/pprint (macroexpand-1 '(attach-contracts bar-c bar [(make-contract-1 [gt0? gt0?] gt0?)] )))
expands like
(clojure.core/defn
 bar-c
 [(make-contract-1 [gt0? gt0?] gt0?)7867]
 (clojure.core/let
  [wrapped-args__7473__auto__
   (clojure.core/map
    dbc.core/wrap-arg-contract
    (dbc.core/zip
     [(make-contract-1 [gt0? gt0?] gt0?)7867]
     [(make-contract-1 [gt0? gt0?] gt0?)]))]
  (clojure.core/apply bar wrapped-args__7473__auto__)))


whilst (clojure.pprint/pprint (macroexpand-1
                        '(attach-contracts
                          add-args-c
                          add-args
...
ore/defn
 ...
 (clojure.core/let
  [wrapped-args__7473__auto__
   (clojure.core/map
    dbc.core/wrap-arg-contract
    (dbc.core/zip [gt0?7876 gt0?7877] [gt0? gt0?]))]
  (clojure.core/apply add-args wrapped-args__7473__auto__)))


The author of a function providing support for say Polycom's HDX
