...
(defmacro attach-contracts-2 [newname func contracts]
  "Create a new function definition `newname' which calls `func' with
   the args stored in arg/contracts wrapped in the corresponding
   contract."
  (let [args (vec (map gensym contracts))]
    `(defn ~newname ~args
       (print (format "wrapped function called with %s\n" ~args))
       (let [wrapped-args# (map wrap-arg-contract (zip ~args ~contracts))]
         (print (format "Applying %s\n" ~func))
         (apply ~func wrapped-args# )))))
...
TODO expand on anaphoric diffiulties encountered with ~'foo
...
#+BEGIN_SRC clojure :tangle yes :exports code
  ...
  (attach-contracts-2 add-args-c
                    add-args
                    [gt0? gt0?])
  
  (deftest add-args-test
    (is (= 8  (add-args-c 4 4)))
    (is (thrown-with-msg? java.lang.Exception #"Contract failed" (add-args-c 0 4))))
