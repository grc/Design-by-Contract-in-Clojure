...
(defn- add-setup-request-impl [type fn]
  (dosync (alter concrete-setup-request conj  {type fn})))
...
on =concrete-setup-request=, appending the map ={type fn}=.  Within
...
functions passed to =add-setup-request-impl=.  We will choose to place
no restriction on the =type= argument: though we currently expect it
to be a keyword by idiom, there's no fundamental reason other types
should not be used so we endeavour to maintain flexibility.

We therefore define a contractual version of
=add-setup-request-impl=, =add-setup-request-impl-c=.  We have
restricted access to the non contractual version by declaring it
private to our name space through the use of =defn-= rather than
=defn=.  It should be noted that this provides protection against
inadvertent rather than malicious invocation. 
