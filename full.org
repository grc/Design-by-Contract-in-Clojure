...
=add-setup-request-impl-c=.  We have restricted access to the non
contractual version by declaring it private to our name space through
the use of =defn-= rather than =defn=.  It should be noted that this
provides protection against inadvertent rather than malicious
invocation.

=add-setup-request-impl= takes two arguments.  We wish to place a
contract of =lenient= on the first; any value will suffice.  
#+latex: \begin{listing}[H]
#+latex: \caption{} 
#+begin_src clojure :tangle yes :exports code

(defn possible-map-index? [key]
  (or (fn? key)
      (keyword? key)))

(attach-contracts add-setup-request-impl-c 
                  add-setup-request-impl
                  [lenient (make-contract foo bar baz)])
