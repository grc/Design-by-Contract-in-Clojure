...
#+begin_src clojure :tangle yes :exports none
  ...
  ;; Experimenting with Ralf's "why postwalk replace?"
  
  (defmacro simple-contract [fn-name arg contract post body]
    (let [wrapped-arg# `(wrap ~contract ~arg "post cond of arg" "pre cond of arg")]
      `(defn ~fn-name [wrapped-arg#]
         ~body)))
  
  (simple-contract grc-save f
                   (make-contract gt0? gt0?)
                   lenient
                   (dosync (ref-set ff-saved f)))
  ...
  (wrapit foo 0 gt0?)*source-path*
  
  
  ;; so we create a function which takes our parameter, wraps iat and calls the original
  (defmacro attach-contract [newname func arg contract ] `(defn ~newname [~arg] (~func (wrap ~contract ~arg "pre" "post")) ))
#+end_src
